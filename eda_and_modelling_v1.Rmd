...
df_test <- df_modelling[-train_index, ]
df_test_no_y <- subset(df_test, select = -convert)

pred_glm <- predict(glm_basic, newdata = df_test_no_y, type = "response")
...
```{r}
df_tree_modelling$convert <- factor(df_tree_modelling$convert)

set.seed(222)
rf_split <- randomForest(
  convert ~ .,
  data = df_tree_modelling[train_index, ],
  ntree = 500,
  mtry  = floor(sqrt(ncol(df_tree_modelling) - 1)),
  importance = TRUE
)
...
# type = prob returns probabilities instead of class labels. This is for several reasons, first is calculating metrics
# like ROC or just probability in general, and second for later decision threshold tuning.
# [, "YES"] means we select the YES probability column.
pred_test <- predict(rf_split, df_tree_modelling[-train_index, ], type = "prob")[, "YES"]
y_test_fac <- factor(ifelse(y_test == 1, "YES", "NO"), levels = c("NO","YES"))
pred_class <- factor(ifelse(pred_test >= 0.5, "YES", "NO"))
confusionMatrix(pred_class, y_test_fac)
roc_auc <- roc(y_test_fac, pred_test)
auc(roc_auc)

f1_at <- function(prob, y, threshold = 0.5){
  y <- factor(ifelse(y_test == 1, "YES", "NO"), levels = c("NO","YES"))
  pred <- factor(ifelse(prob >= threshold, "YES", "NO"), levels = c("NO","YES"))
  tp <- sum(pred == "YES" & y == "YES")
  fp <- sum(pred == "YES" & y == "NO")
  fn <- sum(pred == "NO"  & y == "YES")
  2*tp / (2*tp + fp + fn)
}

f1_05 <- f1_at(pred_test, y_test_fac, 0.5)
f1_05
```
...
opt_lasso <- optimise_threshold(pred_lasso, y_test, metric = "f1")
opt_ridge <- optimise_threshold(pred_ridge, y_test, metric = "f1")
opt_enet <- optimise_threshold(pred_enet, y_test, metric = "f1")
...
```{r}
# use previous rf calculated pred_test to optimise
opt_rf <- optimise_threshold(pred_test, y_test, metric = "f1")

cat("F1 Scores with optimized thresholds:\n")
print(c(Lasso = f1_at(pred_lasso, y_test, opt_lasso$optimal_threshold),
        Ridge = f1_at(pred_ridge, y_test, opt_ridge$optimal_threshold),
        ElasticNet = f1_at(pred_enet, y_test, opt_enet$optimal_threshold),
        RandomForest = f1_at(pred_test, y_test, opt_rf$optimal_threshold)))
...
print(c(Lasso = opt_lasso$optimal_threshold,
        ...
        RandomForest = opt_rf$optimal_threshold))
```

## Up/down sampling
glmnet / cv.glmnet (logistic)
- for family="binomial", glmnet expects a binary outcome. It accepts either numeric 0/1, or a 2-level factor (internally converted to 0/1 with the first level as 0). Turning it into numeric makes it simple and less error prone (0/1 encoding)

randomForest
- in rf, the task is inferred from the type of y. if y is factor, it is classification forest (type="prob" gives class probs as mentioned earlier). if y is numeric, we will get a regression forest (continuous outputs; no type="prob").
...
set.seed(123)
train_up <- upSample(x = X_train, y = y_train_fac, yname = "convert")
X_train_up <- data.matrix(subset(train_up, select = -convert))
y_train_up <- as.numeric(as.character(train_up$convert))

set.seed(123)
train_down <- downSample(x = X_train, y = y_train_fac, yname = "convert")
...
```{r}
pred_lasso_down <- predict(cv_lasso_down, newx = X_test, type = "response", s = "lambda.min")
pred_ridge_down <- predict(cv_ridge_down, newx = X_test, type = "response", s = "lambda.min")
pred_enet_down <- predict(cv_enet_down, newx = X_test, type = "response", s = "lambda.min")
...
## Combined up/down sampling with optimal decision threshold earlier

```{r}
lasso_class_up <- ifelse(pred_lasso_up > opt_lasso$optimal_threshold, 1, 0)
ridge_class_up <- ifelse(pred_ridge_up > opt_ridge$optimal_threshold, 1, 0)
enet_class_up  <- ifelse(pred_enet_up  > opt_enet$optimal_threshold, 1, 0)
...
## RF UP AND DOWN SAMPLING

```{r}
y_train_up_fac   <- factor(y_train_up,   levels = c(0,1), labels = c("NO","YES"))
y_train_down_fac <- factor(y_train_down, levels = c(0,1), labels = c("NO","YES"))
```
...
rf_up <- randomForest(
  ...
  y = y_train_up_fac,
  ntree = 500,
  mtry = floor(sqrt(ncol(X_train_up))),
  importance = TRUE
)
...
rf_down <- randomForest(
  ...
  y = y_train_down_fac,
  ntree = 500,
  mtry = floor(sqrt(ncol(X_train_down))),
  importance = TRUE
)

pred_up <- predict(rf_up, X_test, type = "prob")[, "YES"]
pred_down <- predict(rf_down, X_test, type = "prob")[, "YES"]

f1_up_05 <- f1_at(pred_up, y_test_fac, 0.5)
f1_down_05 <- f1_at(pred_down, y_test_fac, 0.5)

f1_up_05
f1_down_05

f1_up_05 <- f1_at(pred_up, y_test_fac, opt_rf$optimal_threshold)
f1_down_05 <- f1_at(pred_down, y_test_fac, opt_rf$optimal_threshold)
```
