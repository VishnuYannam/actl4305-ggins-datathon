...
### handling the start date < quote create time  ###############################
""" 
df <- df %>%
  mutate(
    valid_start_vs_quote = trip_start_date >= as_date(quote_create_time)
  ) %>%
  filter(!valid_start_vs_quote) %>%
  select(destinations, trip_start_date, trip_end_date, quote_create_time)

df <- df %>%
  mutate(
    quote_create_time = if_else(
      !valid_start_vs_quote & !is.na(quote_create_time),
      make_datetime(
        year = year(quote_create_time),
        month = day(quote_create_time),  # swap month
        day = month(quote_create_time),  # swap day
        hour = hour(quote_create_time),
        min = minute(quote_create_time),
        sec = second(quote_create_time)
      ),
      quote_create_time
    )
  )
...
# Step 2: Extract and count FALSE rows
false_rows_after <- df %>%
  filter(!valid_start_vs_quote)

# Step 3: Print summary
cat("Number of invalid rows after flipping:", nrow(false_rows_after), "\n")
"""
###########################################################################################
...
write.csv(df_write,"C:\\Users\\rohan\\Documents\\UNI\\ACTL4305\\Assignment\\standardised_freely_quote_data_v2.csv", row.names = FALSE)
